...
this.edit(elem);});$(live_selector).addClass("demarcate_editable");$(document).on('DOMNodeInserted',live_selector,function(e){$(e.target).addClass("demarcate_editable");});}}
enable_demarcate_toolbar_handlers();add_edit_placeholder();};demarcate.disable=function(elem){throw new Error("Disabling demarcate not yet implemented - see github issue #16");};demarcate.demarcate=function(elem){var repeatStr=function(str,num)
{return new Array(num+1).join(str);}
var demarkdown_table=function(elem){ var maxColLen=[];var rowLen=0;var cells=[];var op="";var headerRow=true;var col=0;var row=0; elem.find("tr").each(function(){cells[row]=[];col=0; $(this).children().each(function(){ var contents=$(this).text();var contentLen=contents.length; if(maxColLen.length<=col){maxColLen.push(contentLen);}else{if(contentLen>maxColLen[col]){maxColLen[col]=contentLen;}} 
...
demarcate.close_editor=function(elem){var modifyHtml=function(str){ var strippedText=$("<div/>").html(str).text(); var convertor=new Showdown.converter();var op=convertor.makeHtml(strippedText);return op;}
var hide_editor=function(e){e.preventDefault(); $("div#demarcate_toolbar").remove();current_demarcate_editor.remove();current_demarcate_editor=null; if(current_demarcate_element.html()===""&&current_demarcate_element.get(0).tagName!="HR"){current_demarcate_element.remove();}else{current_demarcate_element.removeClass("demarcate_hide");}
current_demarcate_element=null;$(".demarcate_temporary").remove();add_edit_placeholder();}
var demarcate_click_elsewhere_save=function(e){if(current_demarcate_editor==null)return;var tb=$("#demarcate_toolbar");if(!current_demarcate_editor.is(e.target)&&!tb.is(e.target)&&tb.has(e.target).length===0){$("#demarcate_save").click();}}
var replace_tag=function(id){var new_elem=$("<"+id+"/>");current_demarcate_element.after(new_elem);current_demarcate_element.remove();current_demarcate_element=new_elem; $("a#demarcate_cancel").fadeOut('fast',function(){this.remove();}); current_demarcate_editor.css("font",current_demarcate_element.css("font")).css("border",current_demarcate_element.css("border")).css("margin",current_demarcate_element.css("margin")).css("textAlign",current_demarcate_element.css("textAlign")); toolbar_set_active()
current_demarcate_editor.focus();}
var save_and_new_editor_area=function(){ var tag_name=current_demarcate_element.get(0).tagName.toLowerCase();var new_elem=null; if(tag_name=="td"||tag_name=="th"){new_elem=get_next_table_cell(current_demarcate_element);}else{new_elem=$("<"+tag_name+"/>");new_elem.insertAfter(current_demarcate_element);} 
$("#demarcate_save").click();$(".demarcate_temporary").remove(); if(tag_name!="td"&&tag_name!="th"){new_elem.addClass("demarcate_temporary");}
if(new_elem!==null){new_elem.click();}}
var get_next_table_cell=function(elem){var next_td=elem.nextAll("td, th");if(next_td.length==0){var next_tr=elem.parent("tr").nextAll("tr");if(next_tr.length==0){return null;}else{return next_tr.find("th, td").first();}}
return next_td;}
var add_edit_placeholder=function(){demarcate_dom_root.append($('<p />',{class:'demarcate_temporary',text:'Click to add a new block'}));}}
(function($){$.fn.insertAtCaret=function(my_value){ var orig_val=this.val().replace(/\r\n/g,"\n");var start_repl=this.get(0).selectionStart;var end_repl=this.get(0).selectionEnd; var new_val=orig_val.substring(0,start_repl)+my_value+orig_val.substring(end_repl); this.val(new_val.replace(/\n/g,"\r\n"));}})(jQuery);
(function($){$.fn.demarcate=function(){return demarcate.demarcate(this);};})(jQuery);(function($){$.fn.enable_demarcate=function(){return demarcate.enable(this);}})(jQuery);(function($){$.fn.disable_demarcate=function(){return demarcate.disable(this);}})(jQuery);