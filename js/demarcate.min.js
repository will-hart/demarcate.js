...
demarcate.current_editor.focus();});}; var elem_id=elem.attr('id');for(var tag_name in tag_dict){if(tag_dict[tag_name].editable){live_selector="#"+elem_id+
tag_dict[tag_name].selector_type+tag_name;$(document).on('click',live_selector,function(e){ if($("#demarcate_toolbar").has(e.target).length>0||$("#demarcate_toolbar").is(e.target)||$(elem).attr('id')==='demarcate'){return;} 
...
op+="\n";}
return op+"\n\n";}
var demarkdownImage=function(elem){var alt=elem.attr("alt");var title=elem.attr("title");var url=elem.attr("src");var op=" !["+alt+"]("+url;if(title!=""){op+=" \""+title+"\"";}
return op+") ";}
var demarkdown=function(elem,ignore_extras,child_prefix){ var node=elem.get(0);var node_type=node.nodeType;var tag_name=node_type==3?'_text':node.tagName.toLowerCase();var result=""; if(elem.hasClass("demarcate_temporary"))return result; if(!(tag_name in tag_dict))return result; if((!tag_dict[tag_name].markdownable)&&node_type!=3){return result;}
if(tag_name=='div'&&elem.hasClass("toc")){return"\n[TOC]\n\n";}else if(tag_name=='table'){return demarkdownTable(elem);}else if(tag_name=='img'){return demarkdownImage(elem);} 
...
var doSave=function(){ var tag_name=demarcate.current_element.get(0).tagName.toLowerCase();var curr_value=demarcate.current_editor.val();if(curr_value!=""){if(tag_name!="hr"){curr_value=tag_dict[tag_name].prefix+tag_dict[tag_name].childprefix+
curr_value+tag_dict[tag_name].postfix;}}
var new_elem=$(modifyHtml(curr_value)); if(tag_name=="th"||tag_name=="td"){demarcate.current_element.html(new_elem.html());}else{new_elem.insertBefore(demarcate.current_element);demarcate.current_element.remove();demarcate.current_element=new_elem;}}
...
$(".demarcate_style").removeClass("active"); $("#demarcate_"+tag_name).addClass("active");};demarcate.clickElsewhereSave=function(e){if(demarcate.current_editor==null)return;var tb=$("#demarcate_toolbar");if(!demarcate.current_editor.is(e.target)&&!tb.is(e.target)&&tb.has(e.target).length===0){demarcate.close_editor();}};(function($){$.fn.insertAtCaret=function(my_value){ var orig_val=this.val().replace(/\r\n/g,"\n");var start_repl=this.get(0).selectionStart;var end_repl=this.get(0).selectionEnd; var new_val=orig_val.substring(0,start_repl)+my_value+orig_val.substring(end_repl); this.val(new_val.replace(/\n/g,"\r\n"));}})(jQuery);(function($){$.fn.demarcate=function(){return demarcate.demarcate(this);};})(jQuery);(function($){$.fn.enable_demarcate=function(){return demarcate.enable(this);}})(jQuery);(function($){$.fn.disable_demarcate=function(){return demarcate.disable(this);}})(jQuery);