...
if(!ignore_extras||_tag_dict[tag_name].force_prefix){result+=_tag_dict[tag_name].prefix;} 
if(node_type==3){if($.trim(node.nodeValue)==""){return"";}
result+=$.trim(node.nodeValue);} 
result+=_tag_dict[tag_name].childprefix;$.each(elem.contents(),function(index,value){result+=demarkdown($(value),ignore_extras,"");}); if(!ignore_extras||_tag_dict[tag_name].force_prefix){result+=_tag_dict[tag_name].postfix;} 
if(_tag_dict[tag_name].post_newline){result+='\n';} 
...
elem=$(elem);var tag_name=elem.get(0).tagName.toLowerCase(); if(tag_name in _tag_dict){ var md=demarcate.demarcate(elem,true,"");var ed=$("<textarea />",{id:'demarcate'}).css("font",elem.css("font")).css("outline","none").css("margin",elem.css("margin")).css("lineHeight",elem.css("lineHeight")).css("textAlign",elem.css("textAlign"));var tb=generateToolbar();elem.after(tb).slideDown();elem.after(ed).slideDown();elem.addClass("demarcate_hide"); demarcate.current_element=elem;demarcate.current_editor=ed; demarcate.toolbarSetActive(); ed.val($.trim(md)); if(typeof elem.autosize!=undefined){ed.autosize({'append':'\n'});}
...
var doSave=function(){ var tag_name=demarcate.current_element.get(0).tagName.toLowerCase();var curr_value=demarcate.current_editor.val();if(curr_value!=""){if(tag_name!="hr"){curr_value=_tag_dict[tag_name].prefix+_tag_dict[tag_name].childprefix+
